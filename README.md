...
| Stage | Produces |
|---|---|
| `stage_metadata` | 30,031 families, 891 clans, 6,925 unknown-function families, a UniProt structural representative for every family |
...
- [x] **Tightened the unknown-function rule.** Matching the CC abstract as well as the one-line summary looked thorough and flagged 949 characterised families: the Prospector's largest entries were the MurJ lipid II flippase, the ZIP zinc transporter and the bacterial SH3 domain, which undermines the one claim that tab makes. Matching the family's own summary, plus Pfam's DUF and UPF prefixes, gives 6,925. A test asserts those three accessions never reappear.
- [x] **Said conserved domain order in words.** Every co-occurrence row read "100%", which is uninformative and reads like a bug. It is not one: across 71,573 pairs sharing ten or more proteins, 97.7% have an invariant N-to-C order. The invariant case now says "always N-terminal", and a percentage is kept for the 2.3% that genuinely vary, which are the interesting ones.
...
- [x] **App icon, drawn from the real map.** `forge/make_icon.py` renders the actual UMAP projection with real clan colours and the amber query comet, so the icon is the thing the app is about rather than a glyph. It asserts RGB with no alpha (the App Store rejects an alpha channel and PIL hands you RGBA by default) and `verify-bundle.sh` now checks the compiled icon at the bundle root, since an empty appiconset still builds and ships a blank tile.
